...
- **`src/agent/`** — the agent core:
  - `Session.ts` — the first-class context owner: a session holds the `ContextManager`, active `AgentDefinition`, model, cwd, tool history, and agent transitions. `setAgent()` records a transition; `recordTool()` logs every call; `examinedSummary()` produces the deduped "already explored" digest used in hand-offs and folded into the eviction notice (so work survives trimming). `toJSON()`/restore + `SessionStore.ts` persist sessions to `~/.config/ai-cli/sessions/` (autosaved each turn; `iaa sessions`, `iaa chat --resume`).
  ...
  - `ContextManager.ts` — append-only message store, token estimation (3.5 chars/token + per-message overhead), oldest-first trim with an in-context eviction notice (which folds in the work-so-far digest), and proactive compaction at a threshold. `onEvict`/`onUsage` callbacks. `compaction.ts` — pure structured compaction (cap old tool results, stub superseded reads); the runtime adds an optional LLM `foldOlder` summary in `"summarize"` mode.
  - `promptBuilder.ts` — builds the system prompt (rules, permitted-tool descriptions, agent suffix, active skills, native-vs-text tool format, optional few-shot exemplar, and the auto-loaded project-context block).
  - `projectContext.ts` — `findProjectContext()` walks up from the session cwd to the nearest `AGENTS.md` (size-capped); `gitContext.ts` — `getGitContext()` returns branch/changes/recent commits (read-only) for the prompt block + TUI status line. `refreshSystemPrompt()` re-discovers both each turn so they track the cwd/repo state.

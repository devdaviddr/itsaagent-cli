...
- **`src/agent/`** — the agent core:
  ...
  - `projectContext.ts` — `findProjectContext()` walks up from the session cwd to the nearest `AGENTS.md` (size-capped); `refreshSystemPrompt()` re-discovers it each turn so it loads/unloads as the agent `cd`s between projects.

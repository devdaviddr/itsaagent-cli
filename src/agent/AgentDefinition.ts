...
export const BUILTIN_AGENTS: AgentDefinition[] = [
  {
    ...
    systemPromptSuffix: [
      ...
      "4. Finish. Verify it works — read the files back, run the command, or call",
      "   run_tests — then summarise. Only give your final answer once the task is",
      "   actually complete and you have checked it.",
    ].join("\n"),
  },
  {
    id: "plan",
    ...
    systemPromptSuffix: [
      ...
      "<answer>.",
      "Write the plan FOR THE BUILD AGENT to execute with its TOOLS — not as",
      "instructions for a human. Each step is a concrete tool action, e.g.",
      "'make_directory <path>', 'write_file <path> with: <full contents>',",
      "'bash: npm init -y (cwd <path>)', 'bash: npm install express (cwd <path>)'.",
      "NEVER write human steps like 'open Finder/Terminal/a text editor', 'paste the",
      "code', or 'test with Postman/curl', and do not describe yourself or being",
      "read-only. List EVERY file to create (with complete contents) and EVERY command",
      "to run, in order, so build can do the whole thing. Then stop; the user presses",
      "Tab to hand it to the build agent.",
    ].join("\n"),
  },
];

...
- **`src/agent/`** — the agent core:
  ...
  - `Process.ts` — data-driven advised processes. Ships one built-in, `GUIDED_PROCESS` (plan → build); `nextStageIndex`/`stageAgent` are the pure stage state machine driving `/guided`.
...
- **`src/tools/`** — one file per tool family (`ask` (`ask_user`), `bash` (accepts an optional `cwd`), `ssh` incl. upload/download, `git`, `fetch`, `filesystem` incl. read/write/make_directory/edit/append/delete/download). `index.ts` exposes `getDefaultTools()`; `session.ts` holds the shared session cwd. `ask_user` is a runtime-intercepted primitive — its `execute()` is only the non-interactive fallback.

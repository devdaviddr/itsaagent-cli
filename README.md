...
- **Project + git awareness** — in a code folder the agent auto-loads the nearest `AGENTS.md` (conventions, build/test commands) and the **git state** (branch, changed files, recent commits) into its prompt, both refreshed as it works; the TUI status line shows `⎇ branch · n changed`. The `repo_map` tool gives it a structural index (files → functions/classes/exports) to orient before answering codebase questions. (`projectContext`/`gitContext: false` to disable)
...
- **18 built-in tools** — `ask_user`, `bash`, `ssh`, `ssh_upload`, `ssh_download`, `git`, `fetch`, `read_file`, `write_file`, `make_directory`, `edit_file`, `append_file`, `delete_file`, `download_file`, `glob`, `grep`, `run_tests`, `repo_map`
